    ...
    <el-table v-loading="loading" :data="rows">
      ...
      <el-table-column prop="direction" label="方向" width="100">
        <template #default="{ row }">
          <el-tag :type="row.direction === 1 ? 'success' : 'warning'">{{ row.direction === 1 ? '收入' : '支出' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="金额" width="120" />
      <el-table-column prop="balance_after" label="变更后余额" width="140" />
...
async function loadRows() {
  loading.value = true
  try {
    const res = await http.get('/admin/balance-flows', { params: { user_id: userId.value, page: page.value, page_size: pageSize.value } })
